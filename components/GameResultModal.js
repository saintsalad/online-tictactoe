        ...
        <Transition.Root show={open} as={Fragment}>
            <Dialog as="div" className="fixed z-10 inset-0 overflow-y-auto" onClose={() => { }}>
                <div className="xs:flex items-center justify-center min-h-screen px-4 text-center block p-0">
                    ...
                    >
                        <div className="bg-white bg-opacity-10 backdrop-blur-lg border border-white border-opacity-10 relative inline-block  rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:max-w-lg sm:w-full md:my-8 md:max-w-lg md:w-full  lg:my-8 lg:max-w-lg lg:w-full">
                            <div className="bg-transparent px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                                <div className="sm:flex sm:items-start md:flex md:items-start lg:flex lg:items-start">
                                    <RenderEmoji />
                                    <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left md:text-left md:ml-4 md:mt-0 lg:text-left lg:ml-4 lg:mt-0">
                                        <Dialog.Title as="h3" className=" text-lg leading-6 font-medium text-white text-opacity-90">
                                            {modalMsg}
                                        </Dialog.Title>
                                        <div className="mt-2">
                            ...
                            <div className="bg-transparent px-4 py-3 xs:flex xs:flex-row-reverse sm:flex-row-reverse sm:px-6 sm:flex md:flex-row-reverse md:px-6 md:flex lg:flex-row-reverse lg:px-6 lg:flex">
                                <button
                                    type="button"
                                    className="w-full inline-flex justify-center rounded-md border-0 shadow-sm h-fit p-2 bg-gradient-to-tr from-[#F7B12D] via-[#FA8247] to-[#FC585D] text-base font-medium text-white hover:opacity-80 focus:outline-none focus:ring-4 focus:ring-offset-0 focus:ring-[#f7b02d39] sm:ml-3 sm:w-auto md:ml-3 md:w-auto lg:ml-3 lg:w-auto " title='Play Again'
                                    onClick={() => playAgain()}>
                                    <RefreshIcon className="h-7 w-7 text-white text-opacity-90" aria-hidden="true" />
                                </button>
                                <button
                                    type="button"
                                    className="mt-3 w-full inline-flex justify-center rounded-md  backdrop-blur-md shadow-sm h-fit p-2 bg-white bg-opacity-10 text-base font-medium text-white text-opacity-90 hover:bg-opacity-20 focus:outline-none focus:ring-4 focus:ring-offset-0 focus:ring-[#ffffff20] sm:mt-0 md:ml-3 md:w-auto md:mt-0 lg:ml-3 lg:w-auto lg:mt-0 sm:ml-3 sm:w-auto  xs:mt-0 xs:mr-3" title='Exit'
                                    onClick={() => exit()}>
