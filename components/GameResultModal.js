...
export default function GameResultModal(props) {
    ...
    const RenderEmoji = () => {
        ...
        if (win === true) {
            comp.push(
                <div key={uuidv4()} className="mx-auto flex-shrink-0 flex items-center justify-center h-12 w-12 rounded-full bg-green-100 sm:mx-0 sm:h-10 sm:w-10">
                    <EmojiHappyIcon className="h-6 w-6 text-green-600" aria-hidden="true" />
                </div>);
            return comp;
        } else if (win === false) {
            comp.push(
                <div key={uuidv4()} className="mx-auto flex-shrink-0 flex items-center justify-center h-12 w-12 rounded-full bg-red-100 sm:mx-0 sm:h-10 sm:w-10">
